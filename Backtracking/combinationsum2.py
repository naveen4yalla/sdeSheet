class Solution:
     def combinationSum2 (self, candidates, target: int):
        #sort the elements to be in the order to compare the previous values
        candidates.sort()
        #result array to append the values
        res = []
        def backtrack (cur, pos, target):
             #base condition to check if the sum equals zero
             if target == 0:
                 res. append(cur.copy ())
             #exclue the sub trees when they reach negative value
             if target <= 0:
                 return
             prev = -1
